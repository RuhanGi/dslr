...
def trainModel(data, y, headers, n):
	try:
		classes = np.unique(y)
		th = pd.DataFrame(np.zeros((len(classes), n)), columns=list(headers)+['Hogwarts House'], index=classes)

		means = np.mean(data, axis=0)
		stds = np.std(data, axis=0)
		stds[stds == 0] = 1
		ndata = (data - means) / stds
		ndata = np.hstack((ndata, np.ones((ndata.shape[0], 1))))
		...
		for i in range(th.shape[0]):
			...
			denorm_bias = th.iloc[i, -1] - np.sum(weights * means / stds)
			th.iloc[i, :-1] = weights / stds
			th.iloc[i, -1] = denorm_bias
